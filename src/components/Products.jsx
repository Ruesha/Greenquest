...
const Products = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch("https://greenquest-mga3.onrender.com/api/products");
        ...
        {cart.length === 0 ? (
          ...
          <>
            {cart.map((item) => (
              <div
                key={item._id}
                style={{ marginBottom: "1rem", borderBottom: "1px solid #222", paddingBottom: "0.5rem",textAlign:"left", marginTop:"20px" }}
              >
                <h4>{item.name}</h4>
            ...
            <button className="buy-btn" style={{ marginTop: "10px" }}>
              Checkout
            </button>
          </>
        )}
